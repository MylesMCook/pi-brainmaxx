---
name: claude-ruminate
license: MIT
description: Mine older Claude session history for repeated corrections, preferences, missed durable knowledge, and recurring failure patterns in the current repo.
---

# Claude Ruminate

Mine older Claude sessions for durable patterns the current brain has missed.
...
6. Use only the repo-scoped Claude history returned by that command.
...
10. If there is a real durable finding, write a small JSON payload to
    `/tmp/claude-ruminate.json` with this shape:

```json
{
  "findingsSummary": "One durable repo-memory gap was repeated across sessions.",
  "rationale": "This rule appeared in multiple repo-scoped Claude sessions.",
  "changes": [
    {
...
```bash
../../scripts/brainerd-claude.sh stage-ruminate --input /tmp/claude-ruminate.json
```
...
- Ignore `subagents/*.jsonl` in this v1 Claude adapter.
