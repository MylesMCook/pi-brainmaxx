...
const resolveClaudeProjectsRoot = (override) => override ? path.resolve(override) : path.join(os.homedir(), ".claude", "projects");
const listTopLevelSessionFiles = async (projectsRoot) => {
...
const parseJsonLine = (line, file) => {
    try {
        ...
        throw new Error(`Malformed Claude session JSON in ${file}: ${error.message}`);
    }
};
...
export const readClaudeSessionMeta = async (file) => {
    const headerLines = await readHeaderChunk(file);
    for (const line of headerLines) {
        const record = parseJsonLine(line, file);
...
const appendTranscript = (transcriptLines, line, state, maxCharsPerSession) => {
    if (state.truncated) {
        return;
    }
...
const extractClaudeTextParts = (content) => {
...
const extractClaudeToolResults = (content) => {
    if (!Array.isArray(content)) {
        return [];
    }
    const parts = [];
...
const readClaudeSessionTranscript = async (file, maxCharsPerSession) => {
    ...
    for await (const line of stream) {
        ...
        let record;
        try {
            record = parseJsonLine(line, file);
        }
        catch (error) {
...
const sortSessionMeta = (left, right) => {
...
const loadClaudeSessionMetas = async (projectsRoot) => {
...
export const findClaudeProjectDirForRepo = async (projectRoot, projectsRoot) => {
...
export const collectClaudeRepoSessions = async (options) => {
    ...
    const readiness = sessions.length >= minSessions
...
export const collectCurrentClaudeSession = async (options) => {
    const repoRoot = await resolveProjectRoot(options.cwd);
    const resolvedProjectsRoot = resolveClaudeProjectsRoot(options.projectsRoot);
    const loaded = await loadClaudeSessionMetas(resolvedProjectsRoot);
