...
import { collectCodexRepoSessions, collectCurrentCodexSession, } from "./codex-sessions.js";
import { applyCodexRuminateStage, discardCodexRuminateStage, getCodexRuminateStage, stageCodexRuminate, } from "./codex-stage.js";
...
const formatBrainSummary = (projectRoot, created, synced) => {
...
const printInit = async (cwd, applyBootstrap) => {
    ...
    if (result.bootstrap.status === "created") {
        console.log(`Created ${result.bootstrap.noteRelativePath} from ${result.bootstrap.sourceFiles.join(", ")}. Synced: ${result.bootstrap.synced.join(", ") || "none"}.`);
        return;
    }
