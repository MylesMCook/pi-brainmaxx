...
const parseJsonLine = (line, file) => {
    try {
        ...
        throw new Error(`Malformed Codex session JSON in ${file}: ${error.message}`);
    }
};
const readFirstUsableLine = async (file) => {
...
const readSessionMeta = async (file) => {
    const firstLine = await readFirstUsableLine(file);
    const record = parseJsonLine(firstLine, file);
...
const isInjectedContextMessage = (text) => {
...
const findSessionFileForThread = async (sessionsRoot, threadId) => {
...
export const assessCodexRuminationReadiness = (collection, minSessions = DEFAULT_CODEX_MIN_RUMINATION_SESSIONS) => {
...
export const collectCodexRepoSessions = async (options) => {
...
export const collectCurrentCodexSession = async (options) => {
    ...
    if (!isSameOrDescendant(repoRoot, located.meta.cwd)) {
        throw new Error(`Current Codex thread ${currentThreadId} belongs to ${located.meta.cwd}, not repo root ${repoRoot}.`);
    }
    const { transcript, assistantModels, messageCount, warnings } = await readSessionTranscript(located.file, options.maxChars ?? 8_000);
